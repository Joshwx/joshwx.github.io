...
u_kts=ds['u']*1.944
v_kts=ds['v']*1.944
temp_925=temp.metpy.convert_units('degC')
# print(temp_925)
...
speed_925=np.sqrt(u_kts**2+v_kts**2)
#mask settings
mask = speed_925<5
...
advection_calc=metpy.calc.advection(temp_925,u,v,dx=dx,dy=dy)
...
temp_c_smooth=gaussian_filter(temp_925,2)
...
#GH contours
line=ax.contour(lon, lat,gh_smooth,levels=list(range(0, 150, 3)),colors='black',linewidths=3,
                transform=ccrs.PlateCarree())

#wind stuff
wnd_speed= np.arange(25, 85, 5)
speed_925=np.sqrt(u_kts**2+v_kts**2)
# wind=ax.contourf(lon,lat,speed_925, wnd_speed, cmap='turbo', transform=ccrs.PlateCarree())
...
ax.set_title(f'925mb Temperature Advection (10^-5 C/3hr)', fontsize=26, loc='center')
