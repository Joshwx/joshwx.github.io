...
#pull from model
H= Herbie("2026-05-27 16:00", model='rap', product='awp236pgrb')
...
ax.clabel(cf_red,inline=True,colors='red',fontsize=10, fmt='%d')

cf_blue=ax.contour(lon, lat, temp_c, levels=(range(-40,0,2)), colors='blue',linewidths=2,
                   linestyles='dashed',transform=ccrs.PlateCarree())
ax.clabel(cf_blue,inline=True,colors='blue',fontsize=10, fmt='%d')

#dpt contours
cf_lime=ax.contour(lon,lat,dpt_smooth, levels=range(6,10,2), colors='lime',
                   linewidths=2,transform=ccrs.PlateCarree())
ax.clabel(cf_lime,inline=True,colors='lime',fontsize=10, fmt='%d')

cf_green=ax.contour(lon,lat,dpt_smooth, levels=range(10,20,2), colors='forestgreen',linewidths=2,
                    transform=ccrs.PlateCarree())
ax.clabel(cf_green,inline=True,colors='forestgreen',fontsize=10, fmt='%d')
...
ax.set_title(f'850mb Height (dam; black), Temp (C; red), Dewpoint (C; green), Wind (kts)', fontsize=18, loc='center')
